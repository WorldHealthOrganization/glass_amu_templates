...
def load_config() -> configparser.ConfigParser:
    ...
    cfg_path = os.path.join(os.path.curdir, "..\\..", "HAMU_Template", "settings.ini")
    log.debug(cfg_path)
...
def rename_msi_file(orig_path: str, dev_env:str, cfg: configparser.ConfigParser)->str:
    ...
    new_path = f"{path}-{version}v{subversion}-release{ext}" if dev_env=="Release" else f"HAMU_addin-{version}v{subversion}-debug{ext}"
...
def create_zip_file(msi_file: str, dev_env:str, cfg: configparser.ConfigParser):
    ...
    version = cfg["Version"]["Version"]
    subversion = cfg["Version"]["Subversion"]
    zip_path = f"HAMU_installer-{version}v{subversion}-release.zip" if dev_env=="Release" else f"HAMU_installer-{version}v{subversion}_{dt.now():%Y%m%d%H%M}-debug.zip"
